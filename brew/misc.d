...
struct Interval(T) {
    ...
    T length() const                                        { return Math!T.abs(max - min); }
    T clamp(T value) const                                  { return Math!T.clamp(value, min, max); }
    T clampAbove(T value) const                             { return Math!T.clampAbove(value, max); }
    T clampBelow(T value) const                             { return Math!T.clampBelow(value, min); }
}
