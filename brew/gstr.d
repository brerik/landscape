...
struct GStr {
	 ...
	 * copied from phobos
	 */
	static wstring toWString(char *s, size_t len = 0)       { return toWString(Str.toString(s, len)); }
	 ...
	 * copied from phobos
	 */
	static string toString(wchar *s, size_t len = 0) {
		if ( s is null )
	 ...
	 * copied from phobos
	 */
	static char* toStringz(wstring s)                       { return Str.toStringz(toString(s)); }

	static size_t wcslen(wchar *s) {
	    if (s is null)
            return 0;
        size_t len = 0;
        while (s[len] != 0)
